...
class Salb:
    def __init__(self, 
        ...
        self.dependency_graph: nx.DiGraph = self.get_dependency_graph()
    ...
    # Assume `self.precedence_graph` is a DiGraph
        G = self.precedence_graph
        nodes = list(G.nodes)
        n = len(nodes)

        dg = nx.DiGraph()  # dependency graph is undirected
        dg.add_nodes_from(nodes)

        # Check for dependent pairs
        for i in range(n):
            for j in range(i+1, n):
                u, v = nodes[i], nodes[j]
    ...
    def visualize_dependency_graph(self, chosen_tasks: Optional[List[int]] = None):
        # If chosen_tasks is provided, filter the graph to only include the chosen tasks
        if chosen_tasks is not None:
            # Get subgraph with only the chosen tasks
            subgraph = self.dependency_graph.subgraph(chosen_tasks)
        else:
            # Use the entire graph
    ...
    def layered_topo_pos(self, subgraph: nx.DiGraph):
        G_full = subgraph # Full graph with all dependencies
